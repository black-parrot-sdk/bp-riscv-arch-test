...
class blackparrot(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","blackparrot")
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # for blackparrot start building the '--isa' argument. the self.isa is dutnmae specific and may not be
      # useful for all DUTs
...
#	  # echo statement.
#          #if self.target_run:
#          #  # set up the simulation command. Template is for blackparrot. Please change.
...
#The following template only uses shell commands to compile and run the tests.
    ...
    def runTests(self, testList):
      ...
      for testname in testList:
          ...
          test_shortname = testname.split('/')[-1].replace('.S','')

          # name of the elf file after compilation of the test
          elf = test_dir + '/' + test_shortname + '.riscv'
          ...
          #if self.target_run:
          #  # build the command for running the elf on the DUT. In this case we use blackparrot and indicate
          #  # the isa arg that we parsed in the build stage, elf filename and signature filename.
          #  # Template is for blackparrot. Please change for your DUT
          ...
          postprocess = 'touch {0}'.format(sig_file)
