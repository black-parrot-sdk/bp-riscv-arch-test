...
class sail_cSim(pluginTemplate):
    ...
    def initialise(self, suite, work_dir, archtest_env):
        ...
        self.objdump_cmd = 'riscv{1}-unknown-elf-dramfs-objdump -D {0} > {2};'
        self.compile_cmd = 'riscv{1}-unknown-elf-dramfs-gcc -march={0} \
    ...
    def build(self, isa_yaml, platform_yaml):
        ...
        if "D" in ispec["ISA"]:
            self.isa += 'd'
        objdump = "riscv{0}-unknown-elf-dramfs-objdump".format(self.xlen)
        if shutil.which(objdump) is None:
            logger.error(objdump+": executable not found. Please check environment setup.")
            raise SystemExit(1)
        compiler = "riscv{0}-unknown-elf-dramfs-gcc".format(self.xlen)
